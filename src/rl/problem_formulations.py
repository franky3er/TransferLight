...
class GeneraLightProblemFormulation(ProblemFormulation):
    ...
    def get_state(self) -> HeteroData:
        ...
        # Edge Indices
        data["lane_segment", "to", "lane"].edge_index = to_edge_index(
    ...
    def _get_lane_segment_features(self) -> torch.Tensor:
        ...
        for segment in self.net.segments:
            x.append([self.net.get_vehicle_density(segment=segment)])
        return torch.tensor(x, dtype=torch.float32)
        #for lane_id in self.lanes:
